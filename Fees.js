...
router.post('/college-fee/enroll/', async (req, res) => {
    ...
    const amount = parseFloat(Fee);
     const totalamount = parseFloat(Fee);
    const due=Fee-total
    let connection;
    ...
    try {
        ...
        // Check if a payment record already exists
        const checkQuery = `  INSERT INTO Yearly_fees (Regno, Fee_type, Fee_year, Fee_total, Fee_due) 
                  VALUES (?, ?, ?, ?, ?)`;
        const [checkResult] = await connection.query(checkQuery, [Regno, Fee_type, Ayear,total,due]);
...
router.post('/college-fee/online/', async (req, res) => {
    ...
    const { Regno, Fee_type, Fee, Payment_mode, transactionid} = req.body;
    ...
    try {
        ...
        // Insert new payment record
        const insertQuery = `INSERT INTO CollegeFee (Regno, Feetype, PaidFee,  PaymentDate,Payment_mode, transactionid) 
                             VALUES (?, ?, ?, UNIX_TIMESTAMP(),?, ?)`;
        await connection.query(insertQuery, [Regno, Fee_type, Fee, Payment_mode,transactionid]);
